...
def _load_skip_schedule(k: int, sc_prec: int, n_blocks: int = 40,
                        path_override: str = "") -> dict[str, list[int]]:
    """Resolve det's existing skip_worst_K_int{6,7}.json schedules.

    ``path_override`` (optional) lets callers point at a specific schedule
    file (e.g. ``skip_worst_30_int7_l2.json``) without mutating the default
    ``skip_worst_K_int{sens_p}.json`` location.
    """
    if path_override:
        path = Path(path_override)
        if not path.is_absolute():
            path = _DET / path_override
    else:
...
_VALID_SENS_METRICS = ("bbox_ap_drop", "segm_ap_drop", "l2")


def _load_sensitivity_grid(
    sc_prec: int,
    *,
    metric: str = "bbox_ap_drop",
    path: str = "",
) -> dict[tuple[str, int], float]:
    """Load det's per-(op, block) sensitivity score.

    metric ∈ {bbox_ap_drop, segm_ap_drop, l2}. l2 is the per-image backbone
    fingerprint distance vs FP (always ≥ 0, monotone in SC perturbation,
    much smoother than the noisy mAP-drop signals on n=100); only present
    in grids produced by sensitivity_sweep.py with the L2 hook (the
    `_sz1024` files). bbox_ap_drop / segm_ap_drop are signed AP gaps.
    """
    if metric not in _VALID_SENS_METRICS:
        raise ValueError(
            f"unknown --sens_metric {metric!r}; valid: {_VALID_SENS_METRICS}")
    if path:
        p = Path(path) if Path(path).is_absolute() else _DET / path
    else:
        sens_p = 7 if sc_prec >= 7 else 6
        # Prefer the sz1024 file (which carries the l2 column) if it exists;
        # fall back to the legacy 1280 grid for bbox/segm-only consumers.
        candidates = [
            _DET / "sensitivity" / f"sensitivity_int{sens_p}_n100_sz1024.json",
            _DET / "sensitivity" / f"sensitivity_int{sens_p}_n100.json",
        ]
    ...
    for r in data["grid"]:
        ...
        if v is None:
            missing += 1
            continue
        out[(r["op"], int(r["block"]))] = float(v)
    if missing:
        raise KeyError(
            f"sens grid {p} missing {metric!r} for {missing} entries; "
            f"re-run sensitivity_sweep.py to populate it (l2 needs the "
            f"backbone-hook version of the sweep).")
    return out
...
class DetBackend(ms.MPSearchBackend):
    ...
    def __init__(self, *, sc_prec: int, n_search: int, batch_size: int,
                 ...
                 sens_metric: str = "bbox_ap_drop", sens_grid_path: str = "",
                 skip_schedule_path: str = ""):
        ...
        self._sens_metric = sens_metric
        self._sens_grid_path = sens_grid_path
        self._skip_schedule_path = skip_schedule_path
    ...
    def load_sensitivity(self) -> dict[tuple[str, int], float]:
        return _load_sensitivity_grid(
            self._sc_prec,
            metric=self._sens_metric,
            path=self._sens_grid_path,
        )
    ...
    def build_active_ops(self, sc_config: str) -> set[tuple[str, int]]:
        ...
        sched = _load_skip_schedule(k, self._sc_prec, self.n_blocks,
                                    path_override=self._skip_schedule_path)
        return {
            (op, bi)
            for op in SC_OP_NAMES
...
def main():
    ...
    ap.add_argument("--init_mode", default="sensitivity_dp",
                    choices=["sensitivity_dp", "uniform_repair", "map_repair", "ilp"],
                    help="DP over sensitivity (default) maximizes "
                         "Σ sens·SL under the budget — most-sensitive slots "
                         "get high SL, least-sensitive get low SL. "
                         "Combine with --no_search for pure-DP fast path. "
                         "Use 'ilp' for arbitrary-MAC-spread search "
                         "(scipy.optimize.milp; supports qkv_proj+out_proj+qk+av "
                         "all together; pair with --mac_bucket_tol 0.10).")
    ap.add_argument("--ilp_util_mode", default="sens",
                    choices=["sens", "sens_mac", "from_base"],
                    help="ILP utility (default 'sens'): "
                         "'sens' = max Σ sens·ℓ → cost-aware, sens/MAC ranking; "
                         "matches the equal-MAC DP and is the safer default. "
                         "'sens_mac' = max Σ sens·MAC·ℓ → pure-sens ranking, "
                         "ignores cost. Empirically worse on det heterogeneous-MAC "
                         "search (lost ~13 mAP at target=128 vs uniform-128 baseline). "
                         "'from_base' = min Σ (1+sens)·|ℓ−base| → baseline-anchor; "
                         "stay close to --init_level (or --init_sl_map_json) and "
                         "deviate only when sens is high enough to justify. "
                         "Mirrors the equal-MAC DPs uniform_repair / map_repair; "
                         "produces SMOOTH distributions (no LP corner polarization). "
                         "Recommended when the sens grid is noisy "
                         "(det's bbox_ap_drop is single-op-isolation, not multi-op).")
    ap.add_argument("--sens_metric", default="bbox_ap_drop",
                    choices=list(_VALID_SENS_METRICS),
                    help="Per-(op, block) sensitivity score column to read from "
                         "the sweep JSON. 'l2' (backbone-fingerprint distance) "
                         "is always ≥ 0 and much smoother than the noisy "
                         "bbox/segm AP-drop signals on n=100 — recommended "
                         "for --init_mode ilp. Requires the sz1024 sweep "
                         "(sensitivity_int{6,7}_n100_sz1024.json).")
    ap.add_argument("--sens_grid_path", default="",
                    help="Override the sens grid path (relative to det/ or "
                         "absolute). Default: auto-resolve sz1024 then 1280.")
    ap.add_argument("--skip_schedule_path", default="",
                    help="Override the skip_worst_K schedule path (relative "
                         "to det/ or absolute). Default: auto-resolve "
                         "sensitivity/skip/skip_worst_K_int{6,7}.json.")
    ap.add_argument("--ilp_floor_top_k", type=int, default=0,
                    help="ILP only: enforce a hard SL floor (--ilp_floor_min_sl) "
                         "for the top-K most-sensitive search units. 0 = off.")
    ap.add_argument("--ilp_floor_min_sl", type=int, default=192,
                    help="ILP only: floor SL value for top-K floored units.")
    ap.add_argument("--ilp_deep_cut_threshold", type=int, default=32,
                    help="ILP only (from_base): split downgrade slack at this "
                         "threshold (default 32 = the 128→96 step). Cost on "
                         "the first 32 SL of downgrade is (1+sens), beyond "
                         "that is α·(1+sens).")
    ap.add_argument("--ilp_deep_cut_penalty", type=float, default=1.0,
                    help="ILP only (from_base): α multiplier on downgrade "
                         "cost beyond --ilp_deep_cut_threshold. α=1.0 (default) "
                         "= linear (ILP prefers single deep cut to 64). α>1.0 "
                         "= deep cut > 2× shallow → ILP prefers 2× 96 over "
                         "1× 64 except for very low-sens units. α≈1.2 "
                         "recommended for smoother distributions.")
    ...
    ap.add_argument("--no_search", action="store_true",
                    help="Skip iterative_swap_search entirely — output the "
                         "DP-initialized sl_map directly. No model load, no "
                         "calibration forward. Useful for fast sweeps over "
                         "different target_main_sl / sc_config.")
    ...
    ap.add_argument("--mac_bucket_tol", type=float, default=0.0,
                    help="Swap pair MAC tolerance: bucket pairs by "
                         "(MAC_class, |Δ_sl|) where two units share a class "
                         "iff their MACs differ by ≤ this fraction. "
                         "0.0 (default) = exact MAC match (legacy behavior). "
                         "0.10 = 10%% tolerance, recommended with --init_mode ilp "
                         "to allow within-class swaps across mlp/qk_global "
                         "(both ~55-58G).")
    ap.add_argument("--drift_warn_threshold", type=float, default=0.01,
                    help="Warn if cumulative main_sl drift (relative to "
                         "post-init main_sl) exceeds this fraction. "
                         "Only meaningful with --mac_bucket_tol > 0.")
    ...
    backend = DetBackend(
        ...
        sens_metric=args.sens_metric, sens_grid_path=args.sens_grid_path,
        skip_schedule_path=args.skip_schedule_path,
    )
    ...
    sl_map, init_meta = ms.build_initial_sl_map(
        ...
        ilp_util_mode=args.ilp_util_mode,
        ilp_floor_top_k=args.ilp_floor_top_k,
        ilp_floor_min_sl=args.ilp_floor_min_sl,
        ilp_deep_cut_threshold=args.ilp_deep_cut_threshold,
        ilp_deep_cut_penalty=args.ilp_deep_cut_penalty,
    )
    ...
    if args.no_search:
        print(f"[swap_search] --no_search: skipping iterative_swap_search, "
              f"using DP-init sl_map directly", flush=True)
        final_sl_map, history, last_iter = sl_map, [], None
    else:
        ...
        final_sl_map, history, last_iter = ms.iterative_swap_search(
            ...
            mac_bucket_tol=args.mac_bucket_tol,
    ...
    out = {
        "sc_config": args.sc_config,
        "target_main_sl": args.target_main_sl,
        "proxy": "none" if args.no_search else args.proxy,
        "levels": levels,
        "search_ops": sorted(search_ops),
        "fixed_ops": fixed_ops,
        "op_min_levels": op_min_levels,
        "op_max_levels": op_max_levels,
        "n_search": 0 if args.no_search else args.n_search,
        "search_seed": args.search_seed,
        "max_iters": 0 if args.no_search else args.max_iters,
